...
SRC			:=	shell lib lib2 history setup_free parse_tree	\
				arrows main_error str_to_word_array prompt completion \
				func_comp built_in setup_shell alias verif_alias my_strcat_j
...
update_spin:
	...
	@while true; do \
		...
		color=$$(cat .color); \
		printf "\e7\r$$color $$spin $(CLEAR_C)\e8"; \
	done & \
	printf $$! > .make;
...
tests:
	@make -C tests
	@printf "\e[1;3;31;47mTest start"
	@printf "                                                             \
	                                                            $(CLEAR_C)\n"
	@tests/test
	@printf "\e[1;3;31;47mTest done "
	@printf "                                                             \
	                                                            $(CLEAR_C)\n"
	@make -C tests fclean
